/*
  ...
  The version uses a dummy SENSOR class as a demo if there is no sensor device.
  This version specifically emulates an Acurite 609TXC temperature/humidity
  sensor through the transmittor, but the program provides a class for
  ...
  The code to simulate reading a real sensor is modeled after the
  Adafruit BME68x demo program, http://www.adafruit.com/products/3660
...
#include <Wire.h>
#include <SPI.h>
//#include <your sensor library here.h>
...
// SENSOR settings for I2C connection
...
// May need other calls/parameters for your device's library functions
SENSOR sensor;
...
void setup(void) {
  ...
  if (!sensor.begin()) {
    DBG_println("Could not find a valid sensor, check wiring!");
    while (!sensor.begin());
  }

  // Configure your sensor settings here
  sensor.setTemperatureOversampling();
  sensor.setHumidityOversampling();
...
void loop(void) {
  ...
  // Get the readings from the imaginary sensor
  if (!sensor.performReading()) {
    DBG_println(" Sensor failed to perform reading :-(");
    return;
  }

  // Unpack sensor readings, repack for ISM transmission,
  //   and create the waveform
  temp = (uint16_t) ((sensor.temperature+0.05) * 10); // adjust & round
  hum = (uint8_t) (sensor.humidity+0.5);              // round 
  press = (uint16_t) (sensor.pressure / 10.0);        // hPa * 10
  voc = (uint16_t) (sensor.gas_resistance/1000.0) ;   //KOhms
  ...
  DBG_print("\tT=");          DBG_print(sensor.temperature);
  DBG_print(", H=");          DBG_print(sensor.humidity);
  DBG_print(", P=");          DBG_print(sensor.pressure/100.0);
  DBG_print(", V=");          DBG_print(sensor.gas_resistance/1000.0);
  DBG_print(": 0x ");
  hex_print(msg,5);
  DBG_println();
