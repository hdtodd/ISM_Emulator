...
public:
  ...
  void playback() {
	...
	case DEASSERT:
	  digitalWrite(TX,Lo);
	  delayMicroseconds(signals[this_sig].delay_us-22);
	  break;
	default:
...
*/
  SIGNAL TX141TH_signals[6] = {
    {SIG_PULSE,    "Pulse",      838}, //838<-833
    {SIG_SYNC,     "Sync",       1244},  // not used
    {SIG_SYNC_GAP, "Sync-gap",   776},  //776<-833
    {SIG_ZERO,     "Zero",       218}, //218<-208
    {SIG_ONE,      "One",        422}, //422<-417
    {SIG_IM_GAP,   "IM_gap",   10004}
  };

  // Instantiate the device by linking 'signals' to our device timing
  TX141TH() {
    Device_Name = "Lacrosse TX141TH-BV3";
  ...
  void make_wave(uint8_t *msg, uint8_t msgLen) {
    ...
    for (int j=0; j<4; j++) {
    // Preamble
    insert(SIG_PULSE,SIG_PULSE);
    insert(SIG_PULSE,SIG_PULSE);
    insert(SIG_PULSE,SIG_PULSE);
    insert(SIG_PULSE,SIG_PULSE);
    for (int i=0; i<msgLen; i++) {
      ( (uint8_t) ((msg[i/8]>>(7-(i%8))) & 0x01 ) ) == 0 ?
        insert(SIG_ZERO, SIG_ONE ) : insert(SIG_ONE, SIG_ZERO);
      DBG_print(((msg[i/8]>>(7-(i%8)))&0x01));
      };
   // insert(SIG_ONE,SIG_SYNC);
    insert(SIG_PULSE,SIG_PULSE);
    insert(SIG_PULSE,SIG_PULSE);
    };
    cmdList[listEnd++] = {WAIT,SIG_IM_GAP};
    cmdList[listEnd].cmd = DONE;
    DBG_println();
    // Postamble and terminal marker for safety
