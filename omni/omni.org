...
class omni : public ISM_Device {
  ...
  void unpack_msg(uint8_t msg[], uint8_t &type, uint8_t &id, int16_t &iTemp, int16_t &oTemp,
	...
	for (uint8_t j=0; j<REPEAT; j++) {
          ...
          // Preamble
	  for (uint8_t i=0; i<4; i++) insert(SIG_SYNC);

          // The data packet
          if (first) cout << "The msg packet, length=" << (int)msgLen << ", as a series of bits:";

          for (uint8_t i = 0; i < msgLen; i++) {
            insert(((uint8_t)((msg[i / 8] >> (7 - (i % 8))) & 0x01)) == 0 ? SIG_ZERO : SIG_ONE);
            if (first) cout << setw(1) << ((msg[i / 8] >> (7 - (i % 8))) & 0x01);
	  };
        if (first) cout << endl;
        first = false;
	};
...
int main()
{
    ...
    ihum   = (int8_t)   (ihumf + 0.5);         //round with the 0.5
    ohum   = (int8_t)   (ohumf + 0.5);         //round with the 0.5
    press  = (uint16_t) (10.0*pressf + 0.5);   //round with the 0.5
    volts  = (uint16_t) (100.0*(voltsf - 2.50) + 0.5);

    cout << "\nNow create a wave for 'om'" << endl;
    om.pack_msg(type, id, itemp, otemp, ihum, ohum, press, volts, msg);
    cout << "Message as packed for device 'om' in hex: 0x ";
    for (uint8_t j = 0; j < 10; j++) cout << setw(2) << setfill('0') << hex << (int)msg[j] << " ";
    ...
    cout << "Volts\t"  << voltsf      << "\t\t" << ((float) v)/100.0 + 2.50 << endl <<endl;
